...
import tools.nsc.{Settings => CompilerSettings, interpreter, ConsoleWriter, NewLinePrintWriter}
import java.io.{Writer, File}
import tools.nsc.interpreter.{JLineCompletion, Completion, NamedParam, IMain}
...
//                  case Done       => Right( execution.get() )
   ...
   private final class Impl( in: IMain with ResultIntp ) extends Interpreter {
      ...
      def interpret( code: String, quiet: Boolean ) : Interpreter.Result = {
         if( quiet ) {
            in.beQuietDuring( in.interpretWithResult( code, quiet = true ))
         } else {
