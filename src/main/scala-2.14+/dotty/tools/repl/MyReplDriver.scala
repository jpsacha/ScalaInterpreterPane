...
import de.sciss.scalainterpreter.{Completion => IntpCompletion}
import de.sciss.scalainterpreter.{Completion => IntpCompletion}
...
class MyReplDriver(settings: Array[String],
  ...
  private def initialCtx = {
    ...
    val Some(_, ictx) = setup(settings, rootCtx) // ._2
  ...
  /** With cursor info */
  def intpCompletions(expr: String, cursor: Int, state0: State): IntpCompletion.Result = {
    def makeCandidate(completion: Completion) = {
      val displ = completion.label
      // println(s"FOUND '$displ' - $completion")
      IntpCompletion.Simple(displ)
    }
    implicit val state = newRun(state0)
    compiler
      .typeCheck(expr, errorsAllowed = true)
      .map { tree =>
        ...
        given Context = state.context.fresh.setCompilationUnit(unit)
        val srcPos = SourcePosition(file, Span(cursor))
        val (off, completions) = Completion.completions(srcPos)
        // println(s"off = $off")
        val cand = completions.map(makeCandidate)
        IntpCompletion.Result(off, cand)
      }
      .getOrElse(IntpCompletion.Result(cursor, Nil))
  }
